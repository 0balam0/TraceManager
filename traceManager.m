...
for k = 1:length(cF)
   ...
   for i = 1:length(tAssi)
      ...
      for j = 1:length(tAssi(i).sigName)              
        ...
        src = struct('name','', 'value',[], 'unit', '', 'x', [], 'color', [], 'label', '',...
      ...
      % Teoresi          
        ...
        if any(strcmp(fieldnames(tTH_k), sField))
            ...
            if isfield(tTH_k.(sField), 'operation')
                sF1 = tTH_k.(sField).sF;
                tTH_k_new=tTH.(sF1);
                [src.x, src.value] = applicaOffset(tTH_k_new.(sX), tTH_k.(sField), bOrgData);
            else
                [src.x, src.value] = applicaOffset(tTH_k.(sX), tTH_k.(sField), bOrgData);
            end
