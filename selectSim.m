function varargout = selectSim(varargin)
% SELECTSIM M-file for selectSim.fig
%      SELECTSIM, by itself, creates a new SELECTSIM or raises the existing
%      singleton*.
%
%      H = SELECTSIM returns the handle to a new SELECTSIM or the handle to
%      the existing singleton*.
%
%      SELECTSIM('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SELECTSIM.M with the given input arguments.
%
%      SELECTSIM('Property','Value',...) creates a new SELECTSIM or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before selectSim_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to selectSim_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help selectSim

% Last Modified by GUIDE v2.5 06-Feb-2019 15:25:04

% Begin initialization code - DO NOT EDIT
gui_Singleton = 0;
gui_State = struct('gui_Name',       mfilename, ...
   'gui_Singleton',  gui_Singleton, ...
   'gui_OpeningFcn', @selectSim_OpeningFcn, ...
   'gui_OutputFcn',  @selectSim_OutputFcn, ...
   'gui_LayoutFcn',  [] , ...
   'gui_Callback',   []);
...
function selectSim_OpeningFcn(hObject, eventdata, handles, varargin)
% Choose default command line output for selectSim
...
function varargout = selectSim_OutputFcn(hObject, eventdata, handles)
