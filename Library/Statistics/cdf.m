...
if nargin<2
   error('stats:cdf:TooFewInputs','Not enough input arguments'); 
end
...
elseif strcmpi(name,'rayl') | strcmpi(name,'Rayleigh'),
   p = raylcdf(x,a1);
elseif strcmpi(name,'t') | strcmpi(name,'T'),     
   p = tcdf(x,a1);
...
elseif strcmpi(name,'weib') | strcmpi(name,'Weibull') | strcmpi(name,'wbl')
   if strcmpi(name,'weib') | strcmpi(name,'Weibull')
      warning('stats:cdf:ChangedParameters', ['The Statistics Toolbox ' ...
              'uses a new parametrization for the\nWEIBULL distribution'...
              'beginning with release 4.1.']);
   end
   p = wblcdf(x,a1,a2);
else   
