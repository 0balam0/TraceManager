% modificata da petrolo con try-catch per gestire errori
...
function [phat, pci] = statisticaMLE(data,varargin)
...
%   $Revision: 2.14.6.7 $  $Date: 2004/06/25 18:52:53 $
...
% -----aggiunta petrolo end------
        ...
        elseif ischar(dist)
            distNames = {'beta', 'bernoulli', 'binomial', 'exponential', ...
                         'extreme value', 'gamma', 'geometric', 'lognormal', ...
                         'normal', 'negative binomial', 'poisson', ...
                         'rayleigh', 'discrete uniform', 'uniform', 'weibull'};

            i = strmatch(lower(dist), distNames);
            if numel(i) > 1
                error('stats:mle:AmbiguousDistName', ...
                      'Ambiguous distribution name: ''%s''.',dist);
                return
            elseif numel(i) == 1
                dist = distNames{i};
            else
                % it may be an abbreviation that doesn't partially match
                % the name 
                dist = lower(dist);
            end
        else
            error('stats:mle:InvalidDistName', ...
                  ['The value of the ''distribution'' argument must be ', ...
                   'a distribution name.']);
        end
        ...
        if ~isempty(ntrials) && isempty(strmatch(lower(dist), 'binomial'))
            warning('stats:mle:NtrialsNotNeeded', ...
                    ['The number of trials is only valid for the binomial ', ...
                      'distribution.']);
        end
        ...
        case 'beta'
            if ~isempty(cens)
                error('stats:mle:CensoringNotSupported', ...
                      'Censoring is not supported for the beta distribution.');
            end
        ...
        case 'bernoulli'
            if ~isempty(cens)
                error('stats:mle:CensoringNotSupported', ...
                      'Censoring is not supported for the bernoulli distribution.');
            elseif any(data ~= 0 & data ~= 1)
                error('stats:mle:InvalidBernoulliData', ...
        ...
        case 'binomial'
            if isempty(ntrials)
                error('stats:mle:NtrialsNeeded', ...
                      'You must provide the number of trials for a binomial fit.');
            elseif ~isempty(cens)
                error('stats:mle:CensoringNotSupported', ...
                      'Censoring is not supported for the binomial distribution.');
            end
            if ~isempty(freq)
        ...
        case 'geometric'
            if ~isempty(cens)
                error('stats:mle:CensoringNotSupported', ...
                      'Censoring is not supported for the geometric distribution.');
            end
            if ~isempty(freq), data = expandInput(data,freq); end
            phat = 1 ./ (1 + mean(data));
            if nargout > 1
        ...
        case {'negative binomial', 'nbin'}
            if ~isempty(cens)
                error('stats:mle:CensoringNotSupported', ...
                      'Censoring is not supported for the negative binomial distribution.');
            end
            if ~isempty(freq), data = expandInput(data,freq); end
            if nargout < 2
        ...
        case 'poisson'
            if ~isempty(cens)
                error('stats:mle:CensoringNotSupported', ...
                      'Censoring is not supported for the Poisson distribution.');
            end
            if ~isempty(freq), data = expandInput(data,freq); end
            if nargout < 2
                phat = poissfit(data);
        ...
        case {'discrete uniform', 'unid'}
            if ~isempty(cens)
                error('stats:mle:CensoringNotSupported', ...
                      'Censoring is not supported for the discrete uniform distribution.');
            end
            if ~isempty(freq), data = expandInput(data,freq); end
            phat = max(data);
            if nargout > 1
        ...
        case {'weibull', 'wbl'}
            if ~strcmp(dist,'wbl')
                warning('stats:mle:ChangedParameters', ...
                        ['The Statistics Toolbox uses a new parametrization', ...
...
% -----aggiunta petrolo start------
        case {'inversegaussian', 'inverse gaussian'}
        ...
        case {'tlocationscale','t location scale'}
        ...
        case {'non parametric','nonparametric'}
            if nargout<2
               phat = 1;
            else
               phat = 1;
