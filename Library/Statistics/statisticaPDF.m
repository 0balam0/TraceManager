...
function y = statisticaPDF(name,x,varargin)
...
%   $Revision: 2.11.4.3 $  $Date: 2004/07/05 17:03:02 $
...
elseif strcmpi(name,'non parametric') || strcmpi(name,'nonparametric'),    
    y = ksdensity(a,x,'kernel','normal','support','unbounded','function','pdf');
...
elseif strcmpi(name,'weib') || strcmpi(name,'Weibull') || strcmpi(name,'wbl')
    if strcmpi(name,'weib') || strcmpi(name,'Weibull')
       warning('stats:pdf:ChangedParameters', ...
               ['The Statistics Toolbox uses a new parametrization for ', ...
