...
for i = 1:length(cModelName)
   ...
   cRefVars = refreshTunVars(sModel);
    ...
    % Rename, sometimes the file name is wrong
    if exist([sModel '.so'], 'file')
        movefile([sModel '.so'], [sModel ext '.so']);
    end
...
[notfound,warnings] = coder.loadlibrary(libname,hfile,...
            ...
            'includepath',fullfile(matlabroot,'toolbox/physmod/network_engine/c'),...
...
% apro il modello simulink e lo salvo senza link, lavoro solo su quello
% il modello da compilare deve avere il suffisso "_open", che verr�
...
%
% carico le variabili necessarie al modelli Simulink nel wsp base (solo l� va bene)
% fin=length(sModel)-4;   
% sModel=sModel(1:fin);
...
%%% La sostituzione � necessaria perch� altrimenti RSIM compila il modello
%%% di base pi� i modelli reference in un eseguibile bisognoso della
...
%%% il wsp di base verr� persa, pertanto il modello reference prender� le
%%% sue variabili direttamente dal wsp di base (non posso istanziare pi� volte con
...
for j = 1:length(cModRef);
    ...
    load_system(sPar)
    load_system(sModRefName)
    
    % aggiungo il sottosistema che dovr� rimpiazzare il modello reference
            ...
            if ~sign_for_gof
                ...
                if ~isempty(k)
                    str([1:k+1])=[];
                     ...
                     str(k:length(str))=[];
                     if ~isempty(UM)%%%segnale con unit� di misura
                         UM(1)=[];
