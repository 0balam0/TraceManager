% corretta da Petrolo per poter gestire i numeri il 23-5-2005
...
if m==1,
   ...
   s(s==char(0)) = ' ';
   % ---- aggiunta Guenna start------
   casostrano=0;
   if strcmpi(s,'CV');   casostrano=1; end
   if strcmpi(s,'gear'); casostrano=1; end
   if strcmpi(s,'Engine'); casostrano=1; end
   if casostrano
      x = [];
      ok= true; 
   else
...
% Try to convert the string into a number.  If this fails, return [] and
% ok=0 
