...
% idx_map2= find(strncmpi(exp_map{:,1},'"[val]',6));
% idx_map = idx_map1 || idx_map2;
if ~isempty(idx_map)
idx_expand_in = length(c1);
    for k=1:length(idx_map)
        ...
        %Since there are 2 string formats to describe the 1D-Maps, there is a double management of 1D Maps.
        % if the format-string of 1D map is : [val][rpm][0;1]+[row][-][0|1] the switch
        % correctly identifies the kind of map (0D - 1D or 2D). Whereas,
        % the other string format (which is the following: [val][Nm][1;2]+[row][rpm][0|1]+[col][-][]) allowed to describe 1D map,
        % mislead the switch that wrongly identify it as 2D Map.
        % for this reasons, in case 2 (switch that identifies 2D map) it is introduced additional controls on row and col values.
        ...
        switch length(idx_xy)
            ...
            case 2 %case 2-D            
                ...
                check_1d_row = strcmp(row_val,'[]');
                check_1d_col = strcmp(col_val,'[]');
                
                if ~check_1d_row 
                     idx_expand_in=idx_expand_in+1; %in order to add field in the info struct, it is increased the index of total element by one. 
                    %update row name
